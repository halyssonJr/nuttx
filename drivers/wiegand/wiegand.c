...
static int wiegand_start_read_frame (FAR struct wiegand_dev_s *priv, FAR struct wiegand_data_s *data)
{
    int i;
    int ret = OK;
    ...
    for (i = 1; i < facility_index; i++)
        {
            ...
            priv->even = (priv->raw_data[i] ? priv->even++ : priv->even);
        }
    ...
    for (i = facility_index; i < id_index; i++)
        {
            ...
            priv->odd = (priv->raw_data[i] ? priv->odd++ : priv->odd);
        }
